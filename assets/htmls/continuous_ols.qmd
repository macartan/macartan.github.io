...
There's an easy but false intuition that if you randomly assign a continuous treatment, a regression of the outcome on the treatment will get you an estimate of some type of  average treatment effect. 
...
@yitzhaki1996using describes the estimand that *is* targeted, which can indeed be interpreted as a weighted average of local average  effects. But the weights aren't obviously substantively relevant weights. In the case in which $X$ is uniformly assigned (as above), the estimand puts a lot more weight on effects close to the middle of the $X$ range.
