...
The term survey experiments is used to cover a large class of experiments. There is a distinction (see e.g. Cyrus Samii's [post](https://cyrussamii.com/?p=4168) or   [our research design book, Ch 14](https://book.declaredesign.org/library/)) between those that are much like any experiment, aiming to estimate a causal effect of a treatment by manipulating that treatment, and those that specifically use a manipulation, often of survey wording or procedures, to makes it possible measure -- or at least make inferences about -- a descriptive estimand. 

This distinction is based on purpose, and sometimes it is not too clear what the purpose is. 

**Example of a survey experiment for causal inference**
...
**Example of a survey experiment for descriptive inference**. Randomized response surveys in which people are randomly assigned to answer either a sensitive question or a non-sensitive question are typically used for descriptive inference. The goal is to estimate the prevalence of some property of subjects, such as whether people have engaged in illegal behavior. The randomization makes it possible to make inferences about the prevalence of the sensitive behavior while protecting individual privacy. Here the randomization is a tool to make measurement possible, not the focus of interest itself. There is a causal effect of the question on the answer, but the purpose is to make descriptive inferences about something else. 

Sometimes people use the term specifically when focusing on those that aim at descriptive inference. It is complicated though by the fact that for some designs the goal is not obvious and we need more clarity over whether we are in fact interested in description of effect estimation.

The rest illustrates these differences for common types of survey experiment.
